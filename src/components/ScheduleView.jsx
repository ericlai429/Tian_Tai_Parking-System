...
export default function ScheduleView({ scheduleData, setScheduleData }) {
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      {/* 現場執勤表 (極簡純淨版，不強調矩陣，不顯示第一二卡片與最下方注意事項) */}
      <div className="p-6 rounded-2xl border overflow-x-auto space-y-4" 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-3"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded font-bold bg-indigo-500/20 text-indigo-300">
              115年9月份
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 text-xs">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-amber-400/30 border border-amber-400"></span> 週末例假日
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-rose-500/20 text-rose-400 text-center font-bold leading-3">休</span> 排休
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-fuchsia-500/40 border border-fuchsia-400"></span> 9/11 代班
            </span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-center border-collapse min-w-[980px] text-xs font-mono">
            <thead>
              {/* 日期列 1 ~ 30 */}
              <tr className="border-b" style={{ borderColor: 'var(--card-border)', backgroundColor: 'var(--card-hover)' }}>
                <th className="p-2.5 text-left font-sans font-bold w-16 sticky left-0 z-10" style={{ backgroundColor: 'var(--card-hover)', color: 'var(--text)' }}>
                  班別
                </th>
                <th className="p-2.5 text-left font-sans font-bold w-24 sticky left-16 z-10" style={{ backgroundColor: 'var(--card-hover)', color: 'var(--text)' }}>
                  人員
                </th>
            ...
            <tbody>
              {scheduleData.guards.map((guard) => (
                <tr key={guard.id} className="border-b hover:bg-slate-800/10 transition-colors" style={{ borderColor: 'var(--card-border)' }}>
                  {/* 班別 */}
                  <td className="p-2.5 font-sans font-bold text-left sticky left-0 z-10" style={{ backgroundColor: 'var(--card-bg)', color: 'var(--text)' }}>
                  ...
                  {/* 人員姓名 */}
                  <td className="p-2.5 font-sans font-extrabold text-left sticky left-16 z-10" style={{ backgroundColor: 'var(--card-bg)', color: 'var(--text)' }}>
                    {guard.name}
                  </td>
